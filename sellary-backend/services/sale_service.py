...
from schemas.sale import SaleCreate, SaleResponse, SaleItemCreate
from services.calculation_service import CalculationService
from core.state_machine import validate_sale_transition, StateTransitionError
...
class SaleService:
    ...
    def get_all(
        ...
        return [self._to_response(s) for s in sales], total

    def create(self, sale_create: SaleCreate, cashier_id: int) -> SaleResponse:
        """
        Create a sale with full transactional safety.
        Uses row-level locking (SELECT ... FOR UPDATE) to prevent race conditions.
        All operations (stock deduction, sale creation, inventory logs) are atomic.
        """
        try:
            # Step 1: Collect all product IDs and lock them (ordered by ID to prevent deadlocks)
            ...
            items = []
            stock_changes = []  # Track changes for inventory logs

            for item_create in sale_create.items:
                product = product_map[item_create.product_id]
                
                item_subtotal = self.calc.calculate_item_subtotal(
                    item_create.quantity, item_create.unit_price
                )
                item_tax = self.calc.calculate_item_tax(
                    item_subtotal, item_create.tax_percent
                )
                item_total = self.calc.calculate_item_total(
                    item_subtotal, item_tax, item_create.discount_amount
                )
                ...
                # Track stock change (before deducting)
            ...
            if sale_create.customer_id:
                customer = self.customer_repo.get_by_id(sale_create.customer_id)
                if not customer:
            ...
            # Step 5: Create inventory logs (within same transaction)
            ...
            # Step 6: Commit entire transaction
            self.db.commit()
            return self._to_response(sale)
    ...
    def cancel(self, sale_id: int, user_id: int) -> SaleResponse:
        """
        Cancel a sale with full transactional safety.
        Uses row-level locking to restore stock safely.
        """
        try:
            sale = self.sale_repo.get_by_id(sale_id)
            if not sale:
            ...
            # Lock all product rows that need stock restoration
            product_ids = [item.product_id for item in sale.items]
            locked_products = self.product_repo.get_multiple_for_update(product_ids)
            product_map = {p.id: p for p in locked_products}

            # Restore stock and create logs
    ...
    def _to_response(self, sale: Sale) -> SaleResponse:
        # Calculate refunded amount from all returns (graceful fallback if table doesn't exist)
        refunded_amount = Decimal("0.00")
        try:
            for sale_return in getattr(sale, 'returns', []) or []:
                refunded_amount += sale_return.total_refund_amount
        except Exception:
            pass  # Returns table doesn't exist yet
        
        remaining_refundable = sale.total_amount - refunded_amount
        
        # Determine if sale can be returned
            ...
            items=[
                {
                    ...
                    "can_return": (item.quantity - (getattr(item, 'quantity_returned', 0) or 0)) > 0 and can_return,
